    ...
    def _default_amount(self):
        ...
        if active_id:
            ...
            if order.is_insurance_order:
                amount_total = order.patient_share_amount
                # If we refund the entire order, we refund what was paid originally, else we refund the value of the items returned

                if float_is_zero(order.refunded_order_id.patient_share_amount + order.patient_share_amount,
                                 precision_rounding=order.currency_id.rounding):
                    amount_total = -order.refunded_order_id.amount_paid
                print('order.refunded_order_id', order.refunded_order_id, amount_total - order.amount_paid)

                return amount_total - order.amount_paid
            else:
                return super()._default_amount()
        return False